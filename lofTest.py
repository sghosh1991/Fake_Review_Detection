...
out = open(r'C:\Python34\alchemyapi_python\resultLOF.txt','w')

##testing
##out.write("#reviewID;#businessID;Class\n")
...
for line in test:
    ...
    if(headerLine):
        ...
        tokens = line.split(",")
        #temp = [float(x) for x in tokens[2:11]]
        x = [float(tokens[i]) for i in activeFeatureIndex]
        
        #featureVector.append(temp)
        #print(str(type(clf.predict([temp]))))
        instances.append(tuple(x))
